...
class BFOIndividual:
    ...
    def __init__(self, vardim, bound):
        '''
        vardim: dimension of variables，变量的维度
...
class BacterialForagingOptimization:
    '''
    The class for baterial foraging optimization algorithm， 细菌觅食优化算法类
    '''

    def __init__(self, sizepop, vardim, bound, params):
        '''
        sizepop: population sizepop    人口规模
        vardim: dimension of variables   变量维数
        bound: boundaries of variables   变量的边界
        ...
        算法所需参数，它是一个列表，由 [Ned, Nre, Nc, Ns, C, ped, d_attract, w_attract, d_repellant, w_repellant] 组成
        '''
        self.sizepop = sizepop  # 人口规模
        self.vardim = vardim  # 变量维数
        self.bound = bound  # 变量的边界
        self.population = []
        self.bestPopulation = []
    ...
    def solve(self):
        ...
        '''
        self.t = 0
        self.initialize()  # 初始化细菌
        self.evaluate()  # 细菌繁殖和进化
    ...
    def chemotaxls(self):
        '''
        chemotaxls behavior of baterials， 细胞的趋化行为
        '''
        for i in range(0, self.sizepop):
    ...
    def communication(self, ind):
        '''
        cell to cell communication，细胞间通讯
        '''
        Jcc = 0.0
        term1 = 0.0
        term2 = 0.0
    ...
    def reproduction(self):
        ...
        self.sortPopulation()
        newpop = []
        for i in range(0, int(self.sizepop / 2)):
            newpop.append(self.population[i])
        for i in range(int(self.sizepop / 2), self.sizepop):
            self.population[i] = newpop[i - int(self.sizepop / 2)]
